from collections.abc import Iterator
from typing import Any

import phenix_apps.apps.sceptre.configs.infrastructures as infra
import phenix_apps.common.error as error
from phenix_apps.apps.sceptre.configs.registers import Device, Register


def all_registers(
    fd_configs: dict[str, Any],
) -> Iterator[tuple[Any, "Protocol", Register]]:
    """Yield (config, protocol, register) for every register."""

    for config in fd_configs.values():
        for protocol in config.protocols:
            for device in protocol.devices:
                for register in device.registers:
                    yield config, protocol, register


def get_fdconfig_class(infrastructure: str) -> type:
    """A FieldDeviceConfig class bound to one infrastructure, by name."""

    name = infrastructure.lower().strip()

    if name not in infra.INFRASTRUCTURES:
        raise error.AppError(f"Infrastructure: {name} not supported")

    class FieldDeviceConfig(infra.Infrastructure):
        INFRA = name

        def __init__(
            self,
            provider: str,
            name: str,
            interfaces: dict[str, Any],
            devices_by_protocol: dict[str, list[dict[str, Any]]],
            publish_endpoint: str,
            server_endpoint: str,
            device_subtype: str,
            counter: int,
        ) -> None:
            super().__init__()
            self.provider = provider
            self.name = name
            self.ipaddr = interfaces.get("tcp", "")
            self.serial_dev = interfaces.get("serial", "[]")
            self.protocols = self.__generate_protocols(devices_by_protocol)
            self.server_endpoint = server_endpoint
            self.publish_endpoint = publish_endpoint
            self.device_subtype = device_subtype
            self.counter = counter

        def __generate_protocols(
            self, devices_by_protocol: dict[str, list[dict[str, Any]]]
        ) -> list[Protocol]:
            protocols_list = []
            for protocol in devices_by_protocol.keys():
                if "serial" in protocol:
                    protocols_list.append(
                        SerialProtocol(
                            protocol,
                            devices_by_protocol[protocol],
                            type(self),
                            self.serial_dev.pop(0),
                        )
                    )
                else:
                    protocols_list.append(
                        Protocol(
                            protocol,
                            devices_by_protocol[protocol],
                            type(self),
                        )
                    )
            Register.reset_addresses()
            return protocols_list

    return FieldDeviceConfig


class Protocol:
    def __init__(
        self,
        protocol: str,
        devices: list[dict[str, Any]],
        infrastructure_class: type,
    ) -> None:
        self.protocol = protocol
        self.devices = self.__generate_devices(devices, infrastructure_class)

    def __generate_devices(
        self, devices: list[dict[str, Any]], infrastructure_class: type
    ) -> list[Device]:
        devices_list = []
        for device in devices:
            kwargs = {}
            for key in device.keys():
                if key == "type" or key == "name":
                    continue
                kwargs[key] = device[key]
            devices_list.append(
                infrastructure_class.create_device(
                    device["type"], device["name"], self.protocol, **kwargs
                )
            )
        return devices_list


class SerialProtocol(Protocol):
    def __init__(
        self,
        protocol: str,
        devices: list[dict[str, Any]],
        infrastructure_class: type,
        serial_dev: str,
    ) -> None:
        self.serial_dev = serial_dev
        super().__init__(protocol, devices, infrastructure_class)


class OpcConfig:
    def __init__(self, fd_configs: dict[str, Any], opc_ip: str) -> None:
        """Build an OPC configuration object, used to build the OPC
        config file from a mako template.

        Args:
            fd_configs (configs.OpcConfig.Device): A list of field devices to be used in OPC configuration
            opc_ip (string): The IP address of the primary OPC server
        """
        self.channel_list = []
        channels = {}

        for fd_config in fd_configs.values():
            for protocol in fd_config.protocols:
                # One channel per protocol, except BACnet where every device
                # shares a single channel.
                if protocol.protocol == "bacnet":
                    name = "ChannelBACnet"
                else:
                    name = (
                        f"Channel{protocol.protocol.title().replace('-', '_')}"
                        f"{fd_config.name.title().replace('-', '_')}"
                    )

                if name not in channels:
                    channels[name] = OpcConfig.Channel(
                        name, protocol.protocol, opc_ip, fd_config.ipaddr
                    )
                    self.channel_list.append(channels[name])

                device = OpcConfig.Device(
                    fd_config.name,
                    protocol.protocol,
                    fd_config.ipaddr,
                    fd_config.range,
                    fd_config.counter,
                )
                for fd_dev in protocol.devices:
                    for register in fd_dev.registers:
                        device.add_tag(register)

                channels[name].add_device(device)

    class Channel:
        def __init__(self, name: str, protocol: str, opc_ip: str, fd_ip: str) -> None:
            self.name = name
            self.protocol = protocol
            self.opc_ip = opc_ip
            self.fd_ip = fd_ip
            self.devices = []

        def add_device(self, device: "OpcConfig.Device") -> None:
            self.devices.append(device)

    class Device:
        def __init__(
            self,
            fd_name: str,
            protocol: str,
            fd_ip: str,
            range_: tuple[float, float],
            fd_counter: int | None = None,
        ) -> None:
            self.fd_name = fd_name
            self.protocol = protocol
            self.fd_ip = fd_ip
            self.fd_counter = fd_counter
            self.range = range_
            self.tags = []

        def add_tag(self, tag: Register) -> None:
            self.tags.append(tag)


class HistorianConfig:
    def __init__(
        self,
        opc_config: OpcConfig | None = None,
        opc_ip: str = "",
        replication_ips: list[str] | None = None,
        scadaConnectToHistorian: bool = False,
        fields: list[str] | None = None,
    ) -> None:
        """Build a historian configuration object, used to build the historian
        config file from a mako template.

        All arguments are optional.  Any data not available in the configuration
        object will be omitted from the config file

        Args:
            opc_config (configs.OpcConfig): The configuration object built for OPC
            opc_ip (string): The IP address of the primary OPC server
            replication_ips (list of strings): A list of IP addresses for the servers running the backup historians
            scadaConnectToHistorian (Boolean): True if the SCADA server connects to the primary historian
            fields (list of strings): A list of field "types" (i.e. mw, mvar, tank_level).  Only these fields "types"
                                      will be added to a historians configuration from OPC.  If the list is empty
                                      all fields will be added.
        """
        # Dictionary to hold the tags that will be retrieved from OPC and stored in the
        # historian.  Key = TopicName, Value = List of associated tags (points)
        self.tags = {}
        self.opc_ip = opc_ip
        self.replication_ips = replication_ips or []
        self.scadaConnectToHistorian = scadaConnectToHistorian

        if opc_config is not None:
            # Loop through channel list (for channel in opc_config.channel_list) and get each channel name
            for channel in opc_config.channel_list:
                # Loop through each device (for tag in device.devices) and get tag name
                for device in channel.devices:
                    device_name = "Device" + device.fd_name.title().replace("-", "_")
                    tag_list = []
                    for register in device.tags:
                        if not fields or register.field in fields:
                            tagname = f"{register.devname}_{register.regtype}_{register.field}".replace(
                                "-", "_"
                            )
                            tag_list.append(tagname)
                    if tag_list:
                        topic_name = channel.name + "_" + device_name
                        self.tags[topic_name] = tag_list
